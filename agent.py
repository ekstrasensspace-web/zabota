...
Если клиент после цены говорит «интересно», «посмотрю», «может быть», «понятно», «надо подумать», «надо посоветоваться» или просто замолкает — это скрытое возражение. НЕ отпускай. Задай вопрос: "Что вас останавливает?" или "Что поднялось?" и жди ответа. Только после конкретного ответа работай с возражением по скриптам.
...
Закрывай примерно так: "Круг Хранителей знаний — это закрытое сообщество, где Наталья лично ведёт каждого в проявленность: еженедельные эфиры, практики в ковенах, медитации и путь к монетизации своего дара. Что для вас сейчас важнее — практика и рост, или уже выход в монетизацию?" — дождись ответа, потом предлагай тариф и спрашивай про оплату.
...
def fallback_reply_for_message(text):
    ...
    predefined = predefined_reply_for_message(text)
    if predefined:
        return predefined
    # Распознаём курс по ключевым словам — даём релевантный ответ даже без AI
    low = (text or "").lower().strip()
    if any(k in low for k in ["ченнелинг", "ченнел", "ченеленг", "ченелинг", "chaneling", "channeling"]):
        return (
            "Ченнелинг — это обучение общению с ангелами-хранителями, духовными наставниками и вознесёнными учителями напрямую. 🌟\n\n"
            "У нас три тарифа:\n"
            "• Интуит — 15 500 ₽ / 155 €\n"
            "• Проводник — 63 000 ₽ / 770 €\n"
            "• Архитектор Реальности — 109 000 ₽ / 1 170 € (включает Квантовую Разблокировку с Натальей)\n\n"
            "Что для вас важнее всего в этом обучении?"
        )
    if any(k in low for k in ["рейки", "рэйки", "reiki", "рейк", "кундалини"]):
        return (
            "Рейки Бон — это путь целителя: исцеление себя, близких и животных через тибетские практики. ✨\n\n"
            "Начинают все с 1 ступени (15 000 ₽). Оптимальный старт — пакет 1–3 ступени (65 000 ₽).\n"
            "Полный путь 1–9 ступеней — 190 000 ₽ (экономия 75 000 ₽).\n\n"
            "Что вас привлекает в Рейки Бон — исцеление для себя или хотите стать целителем?"
        )
    if any(k in low for k in ["руны", "руна", "рун"]):
        return (
            "Магия Рун — работа с рунными формулами для реализации намерений, проработки кармы и защиты. 🔮\n\n"
            "Есть отдельные модули и полный курс с живой инициацией в Zoom.\n"
            "Что вас интересует в магии рун?"
        )
    if any(k in low for k in ["египет", "египетск"]):
        return (
            "Магия Египта — настоящие тайные знания, которые передавались только посвящённым. 🌙\n\n"
            "Три части: 7 Ключей Египта → 22 Аркана Таро → Малые Арканы.\n"
            "Все три части вместе — 250 000 ₽ (вместо 289 700 ₽ по отдельности).\n\n"
            "Что вас привлекает в египетской традиции?"
        )
    if any(k in low for k in ["друид", "друидов"]):
        return (
            "Магия Друидов — глубокий путь посвящения через все круги силы. 🌿\n\n"
            "Полный курс — 2 250 € (по отдельности 2 750 €, экономия 500 €).\n\n"
            "Что вас привлекает в пути друидов?"
        )
    if any(k in low for k in ["луч", "лучей", "7 луч"]):
        return (
            "7 Лучей Духовного Пробуждения — работа с денежными блоками, родовой кармой и раскрытием силы. ✨\n\n"
            "Для себя: Комбо — 34 900 ₽ / 375 €.\n"
            "Как профессия: Мастер 7 Лучей — 94 900 ₽ / 1 020 €.\n\n"
            "Вы хотите работать с 7 лучами для себя — или сделать это профессией?"
        )
    if any(k in low for k in ["клуб", "хранител", "круг"]):
        return (
            "Круг Хранителей знаний — закрытое сообщество с еженедельными эфирами Натальи, ковенами и путём к монетизации дара. 🌟\n\n"
            "Тарифы: Медитации 1 990 ₽/мес · Голос 5 700 ₽/мес · Маяк 11 400 ₽/мес\n"
            "Годовой Голос — 48 888 ₽ (выгоднее в 1,5 раза).\n\n"
            "Что для вас сейчас важнее — практика или выход в монетизацию?"
        )
    # Общий fallback если курс не распознан
    return (
        "Добрый день! Расскажите подробнее — что вас интересует? "
        "Ченнелинг, Рейки, Руны, 7 Лучей, Магия Египта, Друиды или Клуб Хранителей? 😊"
    )
