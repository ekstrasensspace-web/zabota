...
КОГДА КЛИЕНТ ИЗ РОССИИ И БЕСПОКОИТСЯ О ДОСТУПЕ К ОБУЧЕНИЮ (Telegram заблокирован / Zoom недоступен):
Правильный ответ на такие вопросы:
1. Само обучение проходит на платформе GetCourse (getcourse.ru) — она доступна в России без VPN. Там все уроки, материалы и задания.
2. Чат и канал курса — в Telegram. Telegram иногда работает нестабильно в России, поэтому рекомендуем установить VPN (это бесплатно, например Outline или Lantern). С VPN Telegram работает отлично.
3. Zoom — нужен только для программ где есть живые посвящения от Натальи (прямые эфиры в Zoom). Zoom в России тоже работает через VPN.
4. Если клиент не хочет/не может использовать VPN — подойдут курсы в записи без живых встреч, например отдельные модули без посвящений.
Шаблон ответа: "Само обучение идёт на платформе GetCourse — она работает в России без VPN, там все уроки и материалы. Telegram-чат курса доступен через VPN, установить его несложно и бесплатно. Zoom нужен только для живых посвящений от Натальи — они тоже проходят через VPN. Если хотите, могу подсказать какой курс лучше подойдёт с учётом вашей ситуации?"
